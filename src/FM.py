...
def fm_pass(graph: Graph, verbose=False) -> Graph:
    ...
    for i in range(GRAPH_SIZE):
        ...
        for neighbour in node.adjacent:
             left, right, highest_gain_left, highest_gain_right = update_neighbour(node, neighbour, 
                                                                                   left, right, highest_gain_left, highest_gain_right)
...
def update_neighbour(node: Node, neighbour: Node, left: defaultdict, right: defaultdict,
                      highest_gain_left: int, highest_gain_right: int):
...
def select_node_max_gain(buckets: defaultdict, highest_gain: int) -> Tuple[Node, defaultdict, int]:
...
def remove_from_bucket(node: Node, buckets: defaultdict, highest_gain: int) -> Tuple[defaultdict, int]:
    old_gain = node.gain
...
def update_highest_gain_after_remove(buckets: defaultdict, highest_gain: int, old_gain: int) -> int:
    new_highest_gain = highest_gain
...
def add_to_bucket(node: Node, new_gain: int, buckets: defaultdict, highest_gain: int) -> Tuple[defaultdict, int]:
